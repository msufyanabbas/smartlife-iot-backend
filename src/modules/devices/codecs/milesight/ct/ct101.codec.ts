// src/modules/devices/codecs/milesight/ct101.codec.ts
// Milesight CT101 / CT103 / CT105 — LoRaWAN Smart Current Transformer
//
// Protocol: IPSO channel_id + channel_type
//
// ── Attributes (0xFF channel) ────────────────────────────────────────────────
//   0xFF 0x01 — ipso_version
//   0xFF 0x09 — hardware_version
//   0xFF 0x0A — firmware_version
//   0xFF 0xFF — tsl_version
//   0xFF 0x16 — sn (8B hex)
//   0xFF 0x0F — lorawan_class
//   0xFF 0xFE — reset_event
//   0xFF 0x0B — device_status
//
// ── Telemetry ────────────────────────────────────────────────────────────────
//   0x03 0x97 — total_current (uint32 LE /100, Ah) — cumulative
//   0x04 0x98 — current (uint16 LE /100, A)
//              sentinel 0xFFFF → current_sensor_status = 'read failed'
//   0x09 0x67 — temperature (int16 LE /10, °C)
//              sentinel 0xFFFD → temperature_sensor_status = 'over range alarm'
//              sentinel 0xFFFF → temperature_sensor_status = 'read failed'
//   0x84 0x98 — current alarm: max(u16/100) + min(u16/100) + current(u16/100) + alarm_bits(1B)
//              alarm_bits: bit0=current_threshold_alarm, bit1=current_threshold_alarm_release,
//                          bit2=current_over_range_alarm, bit3=current_over_range_alarm_release
//   0x89 0x67 — temperature alarm: int16/10 + alarm_byte
//              alarm_byte: 0=threshold_alarm_release, 1=threshold_alarm
//
// ── Downlink responses (0xFF / 0xFE channel) ─────────────────────────────────
//   0xFF 0x02 — alarm_report_interval (uint16 LE, minutes)
//   0xFF 0x06 — alarm config (packed data byte):
//              data_byte: bits[5:3] = channel (0x01=current, 0x04=temperature)
//                         bits[2:0] = condition (0=disable,1=below,2=above,3=between,4=outside)
//              followed by threshold_min(u16 or i16/10) + threshold_max + alarm_interval(u16) + alarm_counts(u16)
//              Total: 9 additional bytes after data_byte
//   0xFF 0x10 — reboot echo
//   0xFF 0x27 — clear_current_cumulative echo
//   0xFF 0x28 — report_status echo
//   0xFF 0x8E — report_interval: sub_byte(0x00) + uint16 LE (minutes)
//   0xFF 0xF2 — alarm_report_counts (uint16 LE)
//
// ── Downlink commands ────────────────────────────────────────────────────────
//   0xFF 0x10 0xFF           — reboot
//   0xFF 0x28 0xFF           — report_status
//   0xFF 0x27 0x01           — clear_current_cumulative
//   0xFF 0x8E 0x00 <u16>     — set_report_interval (minutes, 1–1440)
//   0xFF 0x02 <u16>          — set_alarm_report_interval (minutes, 1–1440)
//   0xFF 0xF2 <u16>          — set_alarm_report_counts (1–1000)
//   0xFF 0x06 <data> <min_u16> <max_u16> <interval_u16> <counts_u16> — set_current_alarm_config
//   0xFF 0x06 <data> <min_i16> <max_i16> 0x00 0x00 0x00 0x00         — set_temperature_alarm_config
//
// ── Key protocol notes ───────────────────────────────────────────────────────
//   - report_interval in MINUTES
//   - total_current is cumulative Ah (uint32 /100), not instantaneous
//   - Alarm config data byte packs channel type and condition: (channel<<3)|condition
//     Current channel = 0x01, temperature channel = 0x04
//   - temperature thresholds in alarm config are int16 /10 (stored as ×10)
//
// ── canDecode fingerprint ────────────────────────────────────────────────────
//   0x03 0x97 (total_current) — CT-series exclusive
//   0x04 0x98 (current)       — CT-series exclusive
//   0x84 0x98 (current alarm) — CT-series exclusive

import {
  BaseDeviceCodec,
  DecodedTelemetry,
  EncodedCommand,
} from '../../interfaces/base-codec.interface';

// ── Helpers ──────────────────────────────────────────────────────────────────
function u16(b: number[], i: number): number { return ((b[i + 1] << 8) | b[i]) & 0xffff; }
function i16(b: number[], i: number): number { const v = u16(b, i); return v > 0x7fff ? v - 0x10000 : v; }
function u32(b: number[], i: number): number { return (((b[i + 3] << 24) | (b[i + 2] << 16) | (b[i + 1] << 8) | b[i]) >>> 0); }
function wu16(v: number): number[] { return [v & 0xff, (v >> 8) & 0xff]; }
function wi16(v: number): number[] { const u = v < 0 ? v + 0x10000 : v; return [u & 0xff, (u >> 8) & 0xff]; }

const CONDITION_MAP: Record<number, string> = { 0:'disable', 1:'below', 2:'above', 3:'between', 4:'outside' };
const CONDITION_INV: Record<string, number> = Object.fromEntries(Object.entries(CONDITION_MAP).map(([k, v]) => [v, +k]));

const SENSOR_STATUS: Record<number, string> = { 0:'normal', 1:'over range alarm', 2:'read failed' };

export class MilesightCT101Codec extends BaseDeviceCodec {
  readonly codecId: string         = 'milesight-ct101';
  readonly manufacturer    = 'Milesight';
  readonly supportedModels = ['CT101', 'CT103', 'CT105'];
  readonly protocol        = 'lorawan' as const;
  readonly category        = 'Current Monitoring';
  readonly modelFamily     = 'CT101';
  readonly imageUrl        = 'https://github.com/Milesight-IoT/SensorDecoders/raw/main/ct-series/ct101/ct101-2.png';

  // ── Decode ──────────────────────────────────────────────────────────────────

  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    const bytes   = this.normalizePayload(payload);
    const decoded: any = {};
    let i = 0;

    while (i < bytes.length) {
      const ch = bytes[i++];
      const ty = bytes[i++];

      // ── Attributes ───────────────────────────────────────────────────────────
      if (ch === 0xff && ty === 0x01) {
        const b = bytes[i++]; decoded.ipso_version = `v${(b & 0xf0) >> 4}.${b & 0x0f}`;
      }
      else if (ch === 0xff && ty === 0x09) {
        decoded.hardware_version = `v${(bytes[i] & 0xff).toString(16)}.${(bytes[i + 1] & 0xff) >> 4}`; i += 2;
      }
      else if (ch === 0xff && ty === 0x0a) {
        decoded.firmware_version = `v${(bytes[i] & 0xff).toString(16)}.${(bytes[i + 1] & 0xff).toString(16)}`; i += 2;
      }
      else if (ch === 0xff && ty === 0xff) { decoded.tsl_version = `v${bytes[i]}.${bytes[i + 1]}`; i += 2; }
      else if (ch === 0xff && ty === 0x16) {
        decoded.sn = bytes.slice(i, i + 8).map(b => ('0' + (b & 0xff).toString(16)).slice(-2)).join(''); i += 8;
      }
      else if (ch === 0xff && ty === 0x0f) {
        const cm: Record<number, string> = { 0:'Class A', 1:'Class B', 2:'Class C', 3:'Class CtoB' };
        decoded.lorawan_class = cm[bytes[i]] ?? 'unknown'; i += 1;
      }
      else if (ch === 0xff && ty === 0xfe) { decoded.reset_event   = bytes[i] === 1 ? 'reset' : 'normal'; i += 1; }
      else if (ch === 0xff && ty === 0x0b) { decoded.device_status = bytes[i] === 1 ? 'on' : 'off'; i += 1; }

      // ── Total current (cumulative, uint32 LE /100, Ah) ────────────────────────
      else if (ch === 0x03 && ty === 0x97) {
        decoded.total_current = u32(bytes, i) / 100;
        i += 4;
      }

      // ── Instantaneous current (uint16 LE /100, A) ─────────────────────────────
      else if (ch === 0x04 && ty === 0x98) {
        const raw = u16(bytes, i); i += 2;
        if (raw === 0xffff) {
          decoded.current_sensor_status = SENSOR_STATUS[2]; // read failed
        } else {
          decoded.current = raw / 100;
        }
      }

      // ── Temperature (int16 LE /10, °C) ────────────────────────────────────────
      else if (ch === 0x09 && ty === 0x67) {
        const raw = u16(bytes, i); i += 2;
        if (raw === 0xfffd) {
          decoded.temperature_sensor_status = SENSOR_STATUS[1]; // over range alarm
        } else if (raw === 0xffff) {
          decoded.temperature_sensor_status = SENSOR_STATUS[2]; // read failed
        } else {
          decoded.temperature = (raw > 0x7fff ? raw - 0x10000 : raw) / 10;
        }
      }

      // ── Current alarm (0x84 0x98) ─────────────────────────────────────────────
      // max(u16/100) + min(u16/100) + current(u16/100) + alarm_bits(1B)
      else if (ch === 0x84 && ty === 0x98) {
        decoded.current_max = u16(bytes, i) / 100;
        decoded.current_min = u16(bytes, i + 2) / 100;
        decoded.current     = u16(bytes, i + 4) / 100;
        const alm = bytes[i + 6];
        decoded.current_alarm = {
          current_threshold_alarm:         (alm >> 0) & 1 ? 'yes' : 'no',
          current_threshold_alarm_release:  (alm >> 1) & 1 ? 'yes' : 'no',
          current_over_range_alarm:         (alm >> 2) & 1 ? 'yes' : 'no',
          current_over_range_alarm_release: (alm >> 3) & 1 ? 'yes' : 'no',
        };
        i += 7;
      }

      // ── Temperature alarm (0x89 0x67) ────────────────────────────────────────
      // int16/10 + alarm_byte (0=release, 1=alarm)
      else if (ch === 0x89 && ty === 0x67) {
        decoded.temperature = i16(bytes, i) / 10;
        decoded.temperature_alarm = bytes[i + 2] === 0
          ? 'temperature threshold alarm release'
          : 'temperature threshold alarm';
        i += 3;
      }

      // ── Downlink responses (0xFF / 0xFE channel) ──────────────────────────────
      else if (ch === 0xff || ch === 0xfe) {
        const result = this.handleDownlinkResponse(ty, bytes, i);
        Object.assign(decoded, result.data);
        i = result.offset;
      }

      else { break; }
    }

    return decoded as DecodedTelemetry;
  }

  private handleDownlinkResponse(ty: number, b: number[], offset: number): { data: Record<string, any>; offset: number } {
    const data: Record<string, any> = {};
    switch (ty) {
      case 0x02:
        data.alarm_report_interval = u16(b, offset); offset += 2; break;

      case 0x06: {
        const dataByte    = b[offset];
        const channelType = (dataByte >>> 3) & 0x07;
        const condition   = CONDITION_MAP[dataByte & 0x07] ?? 'unknown';
        if (channelType === 0x01) {
          // Current alarm config — thresholds are uint16 (raw, no /100 here — stored as raw uint16 mA)
          data.current_alarm_config = {
            condition,
            threshold_min:  u16(b, offset + 1),
            threshold_max:  u16(b, offset + 3),
            alarm_interval: u16(b, offset + 5),
            alarm_counts:   u16(b, offset + 7),
          };
        } else if (channelType === 0x04) {
          // Temperature alarm config — thresholds are int16 /10
          data.temperature_alarm_config = {
            condition,
            threshold_min: i16(b, offset + 1) / 10,
            threshold_max: i16(b, offset + 3) / 10,
          };
        }
        offset += 9; break;
      }

      case 0x10: data.reboot = 'yes'; offset += 1; break;
      case 0x27: data.clear_current_cumulative = 'yes'; offset += 1; break;
      case 0x28: data.report_status = 'yes'; offset += 1; break;

      case 0x8e:
        // sub-byte (0x00) + uint16 interval
        data.report_interval = u16(b, offset + 1); offset += 3; break;

      case 0xf2:
        data.alarm_report_counts = u16(b, offset); offset += 2; break;

      default: offset += 1; break;
    }
    return { data, offset };
  }

  // ── Encode ──────────────────────────────────────────────────────────────────

  encode(command: { type: string; params?: any }): EncodedCommand {
    const { type, params = {} } = command;
    let bytes: number[] = [];

    switch (type) {
      case 'reboot':                    bytes = [0xff, 0x10, 0xff]; break;
      case 'report_status':             bytes = [0xff, 0x28, 0xff]; break;
      case 'clear_current_cumulative':  bytes = [0xff, 0x27, 0x01]; break;

      case 'set_report_interval':
        bytes = [0xff, 0x8e, 0x00, ...wu16(params.report_interval ?? 20)]; break;

      case 'set_alarm_report_interval':
        bytes = [0xff, 0x02, ...wu16(params.alarm_report_interval ?? 1)]; break;

      case 'set_alarm_report_counts':
        bytes = [0xff, 0xf2, ...wu16(params.alarm_report_counts ?? 1)]; break;

      // Current alarm config:
      // data_byte = (0x01 << 3) | condition  → channel=1 (current)
      // followed by: threshold_min(u16) + threshold_max(u16) + alarm_interval(u16) + alarm_counts(u16)
      case 'set_current_alarm_config': {
        const p = params;
        const condVal = CONDITION_INV[p.condition ?? 'disable'] ?? 0;
        const dataByte = (0x01 << 3) | condVal;
        bytes = [
          0xff, 0x06, dataByte,
          ...wu16(p.threshold_min ?? 0),
          ...wu16(p.threshold_max ?? 0),
          ...wu16(p.alarm_interval ?? 0),
          ...wu16(p.alarm_counts ?? 0),
        ]; break;
      }

      // Temperature alarm config:
      // data_byte = (0x04 << 3) | condition  → channel=4 (temperature)
      // followed by: threshold_min(i16×10) + threshold_max(i16×10) + 4 reserved zero bytes
      case 'set_temperature_alarm_config': {
        const p = params;
        const condVal = CONDITION_INV[p.condition ?? 'disable'] ?? 0;
        const dataByte = (0x04 << 3) | condVal;
        bytes = [
          0xff, 0x06, dataByte,
          ...wi16(Math.round((p.threshold_min ?? 0) * 10)),
          ...wi16(Math.round((p.threshold_max ?? 0) * 10)),
          0x00, 0x00, 0x00, 0x00, // reserved
        ]; break;
      }

      default:
        throw new Error(`CT101: unsupported command "${type}"`);
    }

    return { fPort: 85, data: this.hexToBase64(this.bytesToHex(bytes)), confirmed: false };
  }

  // ── canDecode ─────────────────────────────────────────────────────────────────
  // CT-series uniquely identified by:
  //   0x03 0x97 — total_current (CT-exclusive type byte)
  //   0x04 0x98 — current (CT-exclusive type byte)
  //   0x84 0x98 — current alarm
  //   0x09 0x67 — temperature (shared, but combined with above makes CT clear)
  //   0x89 0x67 — temperature alarm variant

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      const ch = bytes[i]; const ty = bytes[i + 1];
      if (ch === 0x03 && ty === 0x97) return true; // total_current — most distinctive
      if (ch === 0x04 && ty === 0x98) return true; // current
      if (ch === 0x84 && ty === 0x98) return true; // current alarm
      if (ch === 0x89 && ty === 0x67) return true; // temperature alarm variant
    }
    return false;
  }
}

// CT103 and CT105 share identical wire protocol with CT101.
// These subclasses exist purely for distinct registration in codec.module.ts
// so each model appears as a named entry for users.

export class MilesightCT103Codec extends MilesightCT101Codec {
  override readonly codecId         = 'milesight-ct103';
  override readonly supportedModels = ['CT103'];
}

export class MilesightCT105Codec extends MilesightCT101Codec {
  override readonly codecId         = 'milesight-ct105';
  override readonly supportedModels = ['CT105'];
}