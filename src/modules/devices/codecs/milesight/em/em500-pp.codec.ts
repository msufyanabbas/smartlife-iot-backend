// src/modules/devices/codecs/milesight/em500-pp.codec.ts
/**
 * Milesight EM500-PP — LoRaWAN Pipe Pressure Sensor
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 ...
 *   0x03 0x7B — pressure (int16 LE, kPa)   ← unique fingerprint
 *   0x20 0xCE — history (6B): timestamp(4B u32) + pressure(2B i16, kPa)
 *
 * ── Alarm config 0xFF 0x06 encoding ──────────────────────────────────────────
 *   data[0]: bits[2:0]=condition, bits[4:3]=target (1=pressure), bit6=enable, bit7=alarm_release_enable
 *   data[1-2]: threshold_min (i16LE, kPa)
 *   data[3-4]: threshold_max (i16LE, kPa)
 *   data[5-8]: reserved zeros
 *
 * ── Downlink commands ────────────────────────────────────────────────────────
 *   Identical to EM500-LGT except:
 *   - Calibration uses 0xFF 0xF1 0x03 (pressure target) instead of 0x08 (illuminance)
 *   - No illuminance-specific commands; pressure equivalents replace them
 *
...
export class MilesightEM500PpCodec extends BaseDeviceCodec {
  readonly codecId         = 'milesight-em500-pp';
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // Pressure — int16 LE, kPa (signed: supports vacuum/negative pressure)
      else if (ch === 0x03 && ty === 0x7b) {
        const raw = ((bytes[i+1] << 8) | bytes[i]) & 0xffff;
        decoded.pressure = raw > 0x7fff ? raw - 0x10000 : raw; i += 2;
      }
      // History (6B): timestamp(4B u32) + pressure(2B i16, kPa)
      else if (ch === 0x20 && ty === 0xce) {
        const ts  = (((bytes[i+3] << 24) | (bytes[i+2] << 16) | (bytes[i+1] << 8) | bytes[i]) >>> 0);
        const rawP = ((bytes[i+5] << 8) | bytes[i+4]) & 0xffff;
        const prs  = rawP > 0x7fff ? rawP - 0x10000 : rawP;
        if (!decoded.history) decoded.history = [];
        (decoded.history as any[]).push({ timestamp: ts, pressure: prs });
        i += 6;
      }
  ...
  private decodeDownlinkResponse(
    ...
    switch (ty) {
      ...
      case 0xf1: {
        // target byte (0x03 = pressure), skip it; read enable + i16LE value
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      case 'set_pressure_alarm': {
        const cfg  = p.pressure_alarm_config ?? p;
      ...
      case 'set_pressure_calibration': {
        ...
        bytes = [0xff, 0xf1, 0x03, en, le & 0xff, (le >> 8) & 0xff]; break; // 0x03 = pressure target
      }
      case 'set_d2d_config': {
      ...
      default:
        throw new Error(`EM500-PP: unsupported command "${type}"`);
    }
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // 0x03 0x7B (pressure int16 LE, kPa) — unique to EM500-PP.

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x03 && bytes[i+1] === 0x7b) return true;
    }
    return false;
  }
}