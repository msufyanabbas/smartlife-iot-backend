// src/modules/devices/codecs/milesight/em500-udl.codec.ts
/**
 * Milesight EM500-UDL — LoRaWAN Ultrasonic Distance / Level Sensor
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 ...
 *   0x03 0x82 — distance (uint16 LE, mm)  ← fingerprint (also on EM310-UDL)
 *   0x83 0xE9 — distance mutation alarm (5B):
 *               distance(u16/10, mm) + distance_mutation(u16/10, mm) + alarm_type(1B)
 *               alarm_type: 0=normal, 1=threshold_alarm, 2=mutation_alarm
 *   0x20 0xCE — history (6B): timestamp(4B u32) + distance(2B u16, mm)
 *
 * ── EM500-UDL vs EM310-UDL disambiguation ────────────────────────────────────
 *   Both use 0x03 0x82 for distance. The EM500-UDL additionally has:
 *   - 0xFF attribute channels (ipso_version, hardware_version, etc.)
 *   - 0x83 0xE9 mutation alarm channel
 *   - Richer downlink command set
 *   Use model metadata when available. If only payload is available:
 *   - Presence of 0xFF 0x01 (ipso_version) in payload → EM500-UDL
 *   - Presence of 0x83 0xE9 → EM500-UDL
 *   - Presence of 0xFF 0x1B (measuring_equipment) → not definitive (SWL also has this)
 *   EM310-UDL should be listed AFTER EM500-UDL in ALL_CODECS to allow EM500 to match first
 *   when attribute channels are present.
 *
 * ── Alarm config 0xFF 0x06 encoding ──────────────────────────────────────────
 *   data[0]: bits[2:0]=condition, bits[4:3]=target, bit6=enable, bit7=alarm_release_enable
 *     target: 1=distance threshold, 2=distance mutation
 *   data[1-2]: threshold_min (i16LE, mm)
 *   data[3-4]: threshold_max or mutation_value (u16LE, mm)
 *   data[5-8]: reserved zeros
 *
 * ── Downlink commands ────────────────────────────────────────────────────────
 *   0xFF 0x10 0xFF         — reboot
 *   0xFF 0x28 0xFF         — report_status
 *   0xFF 0x4A 0xFF         — sync_time  (0xFF suffix, confirmed by encoder)
 ...
 *   0xFF 0x06 data(11B)    — distance_alarm_config / distance_mutation_alarm_config
 *   0xFF 0xF5 u8           — alarm_release_enable (global)
 *   0xFF 0xF2 u16LE        — alarm_report_counts
 *   0xFF 0xF1 0x02 enable(1B) i16LE — distance_calibration_settings (mm, no scaling)
 *   0xFF 0x96 ...          — d2d_master_config
 ...
 *   Primary: model metadata (supportedModels includes 'EM500-UDL').
 *   Payload-based: 0x03 0x82 (shared with EM310-UDL); additionally checks for
 *   presence of 0xFF attribute channels which are not present in EM310-UDL
 *   payloads.
 */
...
const DIST_ALARM_MAP: Record<number, string> = { 0:'normal', 1:'threshold_alarm', 2:'mutation_alarm' };
...
export class MilesightEM500UdlCodec extends BaseDeviceCodec {
  readonly codecId         = 'milesight-em500-udl';
  ...
  return {
    ...
    model:        'EM500-UDL',
    description:  'Ultrasonic Distance/Level Sensor (EM500) — distance, mutation alarms, D2D, history',
    telemetryKeys: [
      ...
      { key: 'distance_mutation',label: 'Distance Mutation', type: 'number' as const, unit: 'mm' },
    ],
    commands: [
      ...
      },
      {
        ...
        params: [
          { key: 'enable',               label: 'Enable',              type: 'boolean' as const, required: true  },
          { key: 'alarm_release_enable', label: 'Alarm Release Enable', type: 'boolean' as const, required: false },
          { key: 'mutation_value',        label: 'Mutation (mm)',       type: 'number'  as const, required: false, default: 100 },
        ],
      },
      {
        type:   'set_distance_calibration',
        label:  'Set Distance Calibration',
        params: [
          { key: 'enable',            label: 'Enable',    type: 'boolean' as const, required: true  },
          { key: 'calibration_value', label: 'Value (mm)', type: 'number' as const, required: false, default: 0 },
        ],
      },
      {
    ...
    uiComponents: [
      ...
      { type: 'gauge'   as const, label: 'Distance', keys: ['distance'], unit: 'mm' },
      { type: 'status'  as const, label: 'Distance Alarm', keys: ['distance_alarm'] },
    ],
  };
}
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // Distance mutation alarm (5B): distance(/10)+mutation(/10)+alarm_type
      else if (ch === 0x83 && ty === 0xe9) {
        decoded.distance           = (((bytes[i+1] << 8) | bytes[i]) & 0xffff) / 10;
        decoded.distance_mutation  = (((bytes[i+3] << 8) | bytes[i+2]) & 0xffff) / 10;
        decoded.distance_alarm     = DIST_ALARM_MAP[bytes[i+4] & 0xff] ?? 'unknown';
        i += 5;
      }
      // History (6B): timestamp(4B u32) + distance(2B u16, mm)
      else if (ch === 0x20 && ty === 0xce) {
        const ts   = (((bytes[i+3] << 24) | (bytes[i+2] << 16) | (bytes[i+1] << 8) | bytes[i]) >>> 0);
        const dist = ((bytes[i+5] << 8) | bytes[i+4]) & 0xffff;
        if (!decoded.history) decoded.history = [];
        (decoded.history as any[]).push({ timestamp: ts, distance: dist });
        i += 6;
      }
  ...
  private decodeDownlinkResponse(
    ...
    switch (ty) {
      ...
      case 0x06: {
        ...
        if (id === 1) {
          data.distance_alarm_config = {
            ...
            condition:            CONDITION_MAP[b & 0x07] ?? 'unknown',
            alarm_release_enable: rel === 1 ? 'enable' : 'disable',
            threshold_min:        this.readInt16LE(bytes, offset + 1),
            threshold_max:        this.readInt16LE(bytes, offset + 3),
          };
        } else if (id === 2) {
          data.distance_mutation_alarm_config = {
            enable:               en === 1 ? 'enable' : 'disable',
            alarm_release_enable: rel === 1 ? 'enable' : 'disable',
            mutation_value:       ((bytes[offset+4] << 8) | bytes[offset+3]) & 0xffff,
          };
        }
        offset += 9; break;
      }
      ...
      case 0xf1: {
        // target byte (0x02 = distance), then enable + i16LE calibration value (mm, no scaling)
        data.distance_calibration_settings = {
          enable:            bytes[offset+1] === 1 ? 'enable' : 'disable',
          calibration_value: this.readInt16LE(bytes, offset + 2),
        };
        offset += 4; break;
      }
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      case 'sync_time':     bytes = [0xff, 0x4a, 0xff]; break; // 0xFF suffix confirmed by encoder
      ...
      case 'set_distance_alarm': {
        ...
        const dataByte = (rel << 7) | (en << 6) | (1 << 3) | (cond & 0x07);
        const minRaw = cfg.threshold_min ?? 0; const minLE = minRaw < 0 ? minRaw + 0x10000 : minRaw;
        const maxRaw = cfg.threshold_max ?? 0; const maxLE = maxRaw < 0 ? maxRaw + 0x10000 : maxRaw;
        bytes = [0xff, 0x06, dataByte, minLE & 0xff, (minLE >> 8) & 0xff, maxLE & 0xff, (maxLE >> 8) & 0xff, 0, 0, 0, 0];
        break;
      }
      case 'set_distance_mutation_alarm': {
        const cfg = p.distance_mutation_alarm_config ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const rel = cfg.alarm_release_enable === 'enable' || cfg.alarm_release_enable === 1 ? 1 : 0;
        const dataByte = (rel << 7) | (en << 6) | (2 << 3) | 5; // target=2(mutation), condition=5
        const mut = cfg.mutation_value ?? 0;
        bytes = [0xff, 0x06, dataByte, 0, 0, mut & 0xff, (mut >> 8) & 0xff, 0, 0, 0, 0];
        break;
      }
      ...
      case 'set_distance_calibration': {
        const cfg = p.distance_calibration_settings ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const val = cfg.calibration_value ?? 0; // mm, no scaling
        const le  = val < 0 ? val + 0x10000 : val;
        bytes = [0xff, 0xf1, 0x02, en, le & 0xff, (le >> 8) & 0xff]; break; // 0x02 = distance
      }
      case 'set_d2d_config': {
      ...
      default:
        throw new Error(`EM500-UDL: unsupported command "${type}"`);
    }
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // Primary fingerprint: model metadata.
  // Payload-based: 0x03 0x82 (shared with EM310-UDL). Prefer match when
  // 0xFF attribute channels are also present (e.g. 0xFF 0x01 ipso_version).
  // EM500-UDL should be ordered BEFORE EM310-UDL in ALL_CODECS.

  canDecode(payload: string | Buffer, metadata?: any): boolean {
    if ((metadata as any)?.model) {
      return this.supportedModels.includes((metadata as any).model);
    }
    const bytes = this.normalizePayload(payload);
    let hasDistance = false;
    let hasIpsoOrMutationAlarm = false;
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x03 && bytes[i+1] === 0x82) hasDistance = true;
      if (bytes[i] === 0xff && bytes[i+1] === 0x01)  hasIpsoOrMutationAlarm = true; // ipso_version
      if (bytes[i] === 0x83 && bytes[i+1] === 0xe9)  hasIpsoOrMutationAlarm = true; // mutation alarm
    }
    return hasDistance && hasIpsoOrMutationAlarm;
  }
}