// src/modules/devices/codecs/milesight/em500-swl.codec.ts
/**
 * Milesight EM500-SWL — LoRaWAN Submersible Water Level Sensor
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 ...
 *   0x03 0x77 — depth / water_level (uint16 LE /100, cm)   ← unique fingerprint
 *               Error sentinels: 0xFFFF=collection failed, 0xFFFD=out of range
 *   0xFF 0x1B — measuring_equipment (5B):
 *               rate_byte(1B, bits[2:0]) + range_max(i16) + range_min(i16), in cm
 *               rate: 0=0.01, 1=0.1, 2=1, 3=10, 4=100, 5=1000
 *   0x20 0xCE — history (6B): timestamp(4B u32) + depth(2B u16 /100, cm)
 *               Error sentinels apply in history too.
 *
 * ── Depth scale note ─────────────────────────────────────────────────────────
 *   Wire value is uint16 LE in units of 0.01 cm (i.e. value / 100 = cm).
 *   Alarm thresholds are encoded as int16 LE × 100 (cm → wire).
 *   Calibration value encoded as int16 LE × 100.
 *
 * ── Downlink commands ────────────────────────────────────────────────────────
 *   0xFF 0x10 0xFF         — reboot
 *   0xFF 0x28 0xFF         — report_status
 *   0xFF 0x4A 0x00         — sync_time  (note: 0x00 suffix, like PT100)
 ...
 *   0xFF 0x06 data(11B)    — depth_alarm_config
 *   0xFF 0xF5 u8           — alarm_release_enable (global)
 *   0xFF 0xF2 u16LE        — alarm_report_counts
 *   0xFF 0xF1 0x06 enable(1B) i16LE×100 — depth_calibration_settings
 *   0xFF 0x96 mode(1B) enable(1B) lora_uplink_enable(1B) d2d_cmd(2B) +3B — d2d_master_config
...
export class MilesightEM500SwlCodec extends BaseDeviceCodec {
  readonly codecId         = 'milesight-em500-swl';
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // Measuring equipment (shares 0xFF 0x1B with EM500-PT100 but units are cm here)
      ...
      // Depth / water level — uint16 LE /100, cm. 0xFFFF=failed, 0xFFFD=out of range
      else if (ch === 0x03 && ty === 0x77) {
        const raw = ((bytes[i+1] << 8) | bytes[i]) & 0xffff;
        if      (raw === 0xffff) decoded.depth_error = 'collection_failed';
        else if (raw === 0xfffd) decoded.depth_error = 'out_of_range';
        else                     decoded.depth = raw / 100;
        i += 2;
      }
      // History (6B): timestamp(4B u32) + depth(2B u16 /100, cm)
      else if (ch === 0x20 && ty === 0xce) {
        const ts  = (((bytes[i+3] << 24) | (bytes[i+2] << 16) | (bytes[i+1] << 8) | bytes[i]) >>> 0);
        const raw = ((bytes[i+5] << 8) | bytes[i+4]) & 0xffff;
        const entry: Record<string, any> = { timestamp: ts };
        if      (raw === 0xffff) entry.depth_error = 'collection_failed';
        else if (raw === 0xfffd) entry.depth_error = 'out_of_range';
        else                     entry.depth = raw / 100;
  ...
  private decodeDownlinkResponse(
    ...
    switch (ty) {
      ...
      case 0x06: {
        ...
        const id     = (b >> 3) & 0x07;
        if (id === 1) {
          data.depth_alarm_config = {
            enable:        ((b >> 6) & 1) === 1 ? 'enable' : 'disable',
      ...
      case 0xf1: {
        // target byte (0x06 = depth)
        if ((bytes[offset] & 0xff) === 0x06) {
          data.depth_calibration_settings = {
            enable:            bytes[offset+1] === 1 ? 'enable' : 'disable',
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      // Note: EM500-SWL uses 0x00 suffix (like PT100), not 0xFF
      ...
      case 'set_depth_alarm': {
        const cfg  = p.depth_alarm_config ?? p;
        ...
        const minRaw = Math.round((cfg.threshold_min ?? 0) * 100);
        const maxRaw = Math.round((cfg.threshold_max ?? 0) * 100);
      ...
      case 'set_depth_calibration': {
        const cfg = p.depth_calibration_settings ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const val = Math.round((cfg.calibration_value ?? 0) * 100);
        const le  = val < 0 ? val + 0x10000 : val;
        bytes = [0xff, 0xf1, 0x06, en, le & 0xff, (le >> 8) & 0xff]; break; // 0x06 = depth target
      }
      case 'set_d2d_config': {
      ...
      default:
        throw new Error(`EM500-SWL: unsupported command "${type}"`);
    }
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // 0x03 0x77 (depth/water_level uint16 /100, cm) — unique to EM500-SWL.

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x03 && bytes[i+1] === 0x77) return true;
    }
    return false;
  }
}