// src/modules/devices/codecs/milesight/em500-co2.codec.ts
/**
 * Milesight EM500-CO2 — LoRaWAN CO2 / Temperature / Humidity / Pressure Sensor
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 ...
 *   0x05 0x7D — co2 (uint16 LE, ppm)                   ← fingerprint
 *   0x06 0x73 — pressure (uint16 LE /10, hPa)
 ...
 *   0x20 0xCE — history (11B): timestamp(4B u32) + co2(2B u16) + pressure(2B u16/10)
 *                              + temperature(2B i16/10) + humidity(1B u8/2)
 *
 * ── Alarm config 0xFF 0x06 encoding ──────────────────────────────────────────
 *   data[0]: bits[2:0]=condition, bits[5:3]=target, bit6=enable
 *     target: 1=CO2, 2=temperature threshold, 3=temperature mutation
 *   data[1-2]: threshold_min (u16LE for CO2, i16LE×10 for temp)
 *   data[3-4]: threshold_max or mutation
 ...
 *   0xFF 0x1A mode(1B) [u16LE] — co2_calibration_config
 *   0xFF 0x39 enable(1B) period(u16LE) value(u16LE) — co2_abc_calibration_schedule
 *   0xFF 0xF1 target(1B) enable(1B) i16LE — calibration value (temp=0x00, CO2=0x04, pressure=0x05, humidity=0x09)
 *   0xFF 0x87 mode(1B) i16LE — altitude_calibration_settings
 *   0xFF 0x18 sensor_id(1B) enable(1B) — sensor_function_config
 *   0xFF 0x06 data(11B)    — alarm config
 *   0xFF 0xF2 u16LE        — alarm_report_counts
 *   0xFF 0xF5 u8           — alarm_release_enable
 *   0xFF 0x96 mode(1B) enable(1B) lora_uplink_enable(1B) d2d_cmd(2B) +3B reserved — d2d_master_config
 *   0xFF 0x35 key(8B)      — d2d_key
 *   0xFF 0x84 u8           — d2d_enable
 *   0xFF 0x69 u8           — retransmit_enable
 *   0xFF 0x6A 0x00 u16LE   — retransmit_interval
 *   0xFF 0x6A 0x01 u16LE   — resend_interval
...
const CAL_MODE: Record<number, string> = { 0:'factory', 1:'abc', 2:'manual', 3:'background', 4:'zero' };
const CAL_MODE_R: Record<string, number> = { factory:0, abc:1, manual:2, background:3, zero:4 };

const ALT_MODE: Record<number, string> = { 0:'disable', 1:'auto', 2:'manual' };
const ALT_MODE_R: Record<string, number> = { disable:0, auto:1, manual:2 };
...
export class MilesightEM500Co2Codec extends BaseDeviceCodec {
  readonly codecId         = 'milesight-em500-co2';
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // History (11B): timestamp(4) + co2(2) + pressure(2/10) + temperature(2/10) + humidity(1/2)
      else if (ch === 0x20 && ty === 0xce) {
        const ts  = (((bytes[i+3] << 24) | (bytes[i+2] << 16) | (bytes[i+1] << 8) | bytes[i]) >>> 0);
        const co2 = ((bytes[i+5] << 8) | bytes[i+4]) & 0xffff;
        const prs = (((bytes[i+7] << 8) | bytes[i+6]) & 0xffff) / 10;
        const rawT = (bytes[i+9] << 8) | bytes[i+8];
        const temp = (rawT > 0x7fff ? rawT - 0x10000 : rawT) / 10;
        const hum = (bytes[i+10] & 0xff) / 2;
        if (!decoded.history) decoded.history = [];
        (decoded.history as any[]).push({ timestamp: ts, co2, pressure: prs, temperature: temp, humidity: hum });
        i += 11;
      }
  ...
  private decodeDownlinkResponse(
    ...
    switch (ty) {
      ...
      case 0x18: {
        ...
        const val  = bytes[offset+1] & 0xff;
        const sensorKeys: Record<number, string> = { 1:'temperature_enable', 2:'humidity_enable', 5:'co2_enable', 6:'pressure_enable' };
        const cfg: Record<string, any> = {};
        if (mode === 0) {
          for (const [k, key] of Object.entries(sensorKeys)) {
            cfg[key] = ((val >> Number(k)) & 1) === 1 ? 'enable' : 'disable';
          }
        } else if (sensorKeys[mode]) {
          cfg[sensorKeys[mode]] = val === 1 ? 'enable' : 'disable';
        }
        data.sensor_function_config = cfg;
        offset += 2; break;
      }
      case 0x1a: {
        const modeVal = bytes[offset] & 0xff;
        data.co2_calibration_config = { mode: CAL_MODE[modeVal] ?? 'unknown' };
        if (modeVal === 2) {
          data.co2_calibration_config.calibration_value = ((bytes[offset+2] << 8) | bytes[offset+1]) & 0xffff;
          offset += 3;
        } else {
          offset += 1;
        }
      ...
      case 0x39: {
        data.co2_abc_calibration_schedule = {
      ...
      case 0x87: {
        data.altitude_calibration_settings = {
          mode:              ALT_MODE[bytes[offset] & 0xff] ?? 'unknown',
          calibration_value: this.readInt16LE(bytes, offset + 1),
        };
        offset += 3; break;
      }
      case 0x96: {
        const d2d: Record<string, any> = {
          ...
          d2d_cmd:            ((bytes[offset+3] & 0xff).toString(16).padStart(2,'0') + (bytes[offset+4] & 0xff).toString(16).padStart(2,'0')).toUpperCase(),
        };
        offset += 8;
      ...
      case 0xf1: {
        const target = bytes[offset] & 0xff;
        const en     = bytes[offset+1] === 1 ? 'enable' : 'disable';
        const val    = this.readInt16LE(bytes, offset + 2);
        if      (target === 0x00) data.temperature_calibration_settings = { enable: en, calibration_value: val / 10 };
        else if (target === 0x04) data.co2_calibration_settings         = { enable: en, calibration_value: val };
        else if (target === 0x05) data.pressure_calibration_settings    = { enable: en, calibration_value: val / 10 };
        else if (target === 0x09) data.humidity_calibration_settings    = { enable: en, calibration_value: val / 2 };
        offset += 4; break;
      }
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      case 'set_co2_calibration': {
        const cfg  = p.co2_calibration_config ?? p;
        const mode = typeof cfg.mode === 'string' ? (CAL_MODE_R[cfg.mode] ?? 0) : (cfg.mode ?? 0);
        if (mode === 2) {
          const val = cfg.calibration_value ?? 400;
          bytes = [0xff, 0x1a, mode, val & 0xff, (val >> 8) & 0xff];
        } else {
          bytes = [0xff, 0x1a, mode & 0xff];
        }
        break;
      }
      case 'set_co2_abc_calibration': {
        const cfg = p.co2_abc_calibration_schedule ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const per = cfg.period ?? 0;
        const cal = cfg.calibration_value ?? 400;
        bytes = [0xff, 0x39, en, per & 0xff, (per >> 8) & 0xff, cal & 0xff, (cal >> 8) & 0xff]; break;
      }
      case 'set_sensor_function': {
        const cfg = p.sensor_function_config ?? p;
        const sensorIds: Record<string, number> = { temperature_enable:1, humidity_enable:2, co2_enable:5, pressure_enable:6 };
        for (const [key, id] of Object.entries(sensorIds)) {
          if (key in cfg) {
            const en = cfg[key] === 'enable' || cfg[key] === 1 ? 1 : 0;
            bytes.push(0xff, 0x18, id, en);
          }
        }
        break;
      }
      case 'set_temperature_calibration': {
        ...
        bytes = [0xff, 0xf1, 0x00, en, le & 0xff, (le >> 8) & 0xff]; break;
      }
      case 'set_co2_calibration_value': {
        const cfg = p.co2_calibration_settings ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const val = cfg.calibration_value ?? 0;
        bytes = [0xff, 0xf1, 0x04, en, val & 0xff, (val >> 8) & 0xff]; break;
      }
      case 'set_pressure_calibration': {
        const cfg = p.pressure_calibration_settings ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const val = Math.round((cfg.calibration_value ?? 0) * 10);
        const le  = val < 0 ? val + 0x10000 : val;
        bytes = [0xff, 0xf1, 0x05, en, le & 0xff, (le >> 8) & 0xff]; break;
      }
      case 'set_humidity_calibration': {
        const cfg = p.humidity_calibration_settings ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const val = Math.round((cfg.calibration_value ?? 0) * 2);
        const le  = val < 0 ? val + 0x10000 : val;
        bytes = [0xff, 0xf1, 0x09, en, le & 0xff, (le >> 8) & 0xff]; break;
      }
      case 'set_altitude_calibration': {
        const cfg  = p.altitude_calibration_settings ?? p;
        const mode = typeof cfg.mode === 'string' ? (ALT_MODE_R[cfg.mode] ?? 0) : (cfg.mode ?? 0);
        const val  = cfg.calibration_value ?? 0;
        const le   = val < 0 ? val + 0x10000 : val;
        bytes = [0xff, 0x87, mode & 0xff, le & 0xff, (le >> 8) & 0xff]; break;
      }
      case 'set_co2_alarm': bytes = this.encodeAlarmConfig(1, false, p.co2_alarm_config ?? p); break;
      case 'set_temperature_alarm': bytes = this.encodeAlarmConfig(2, true, p.temperature_alarm_config ?? p); break;
      case 'set_temperature_mutation_alarm': bytes = this.encodeMutationAlarm(p.temperature_mutation_alarm_config ?? p); break;
      ...
      case 'set_d2d_key': {
        const key = (p.d2d_key ?? '0000000000000000').replace(/[^0-9A-Fa-f]/g,'').padEnd(16,'0').slice(0,16);
        const keyBytes = Array.from({length:8}, (_,j) => parseInt(key.slice(j*2, j*2+2), 16));
      ...
      default:
        throw new Error(`EM500-CO2: unsupported command "${type}"`);
    }

    return { fPort: 85, data: this.hexToBase64(this.bytesToHex(bytes)), confirmed: false };
  }

  private encodeAlarmConfig(target: number, tempScale: boolean, cfg: any): number[] {
    const cond   = typeof cfg.condition === 'string' ? (CONDITION_R[cfg.condition] ?? 0) : (cfg.condition ?? 0);
    const enable = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
    const dataByte = (enable << 6) | ((target & 0x07) << 3) | (cond & 0x07);
    const minRaw = tempScale ? Math.round((cfg.threshold_min ?? 0) * 10) : (cfg.threshold_min ?? 0);
    const maxRaw = tempScale ? Math.round((cfg.threshold_max ?? 0) * 10) : (cfg.threshold_max ?? 0);
  ...
  private encodeMutationAlarm(cfg: any): number[] {
    const enable   = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
    const dataByte = (enable << 6) | (3 << 3) | 5; // target=3 (temp mutation), condition=5 (mutation)
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // 0x05 0x7D (CO2 ppm) is unique to EM500-CO2.

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x05 && bytes[i+1] === 0x7d) return true;
    }
    return false;
  }
}