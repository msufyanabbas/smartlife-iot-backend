// src/modules/devices/codecs/milesight/em500-lgt.codec.ts
/**
 * Milesight EM500-LGT — LoRaWAN Light / Illuminance Sensor
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 ...
 *   0x03 0x94 — illumination (uint32 LE, lux)  ← unique fingerprint
 *   0x20 0xCE — history (8B): timestamp(4B u32) + illumination(4B u32, lux)
 *
 * ── Alarm config 0xFF 0x06 encoding ──────────────────────────────────────────
 *   data[0]: bits[2:0]=condition, bits[4:3]=target (1=illuminance), bit6=enable, bit7=alarm_release_enable
 *   data[1-2]: threshold_min (i16LE, lx)
 *   data[3-4]: threshold_max (i16LE, lx)
 ...
 *   0xFF 0x06 data(11B)    — illuminance_alarm_config
 *   0xFF 0xF5 u8           — alarm_release_enable (global)
 *   0xFF 0xF2 u16LE        — alarm_report_counts
 *   0xFF 0xF1 0x08 enable(1B) i16LE — illuminance_calibration_settings
...
export class MilesightEM500LgtCodec extends BaseDeviceCodec {
  readonly codecId         = 'milesight-em500-lgt';
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // Illumination — uint32 LE, lux
      else if (ch === 0x03 && ty === 0x94) {
        decoded.illumination = (((bytes[i+3] << 24) | (bytes[i+2] << 16) | (bytes[i+1] << 8) | bytes[i]) >>> 0);
        i += 4;
      }
      // History (8B): timestamp(4B u32) + illumination(4B u32)
      else if (ch === 0x20 && ty === 0xce) {
        const ts  = (((bytes[i+3] << 24) | (bytes[i+2] << 16) | (bytes[i+1] << 8) | bytes[i]) >>> 0);
        const lux = (((bytes[i+7] << 24) | (bytes[i+6] << 16) | (bytes[i+5] << 8) | bytes[i+4]) >>> 0);
        if (!decoded.history) decoded.history = [];
        (decoded.history as any[]).push({ timestamp: ts, illumination: lux });
        i += 8;
      }
  ...
  private decodeDownlinkResponse(
    ...
    switch (ty) {
      ...
      case 0x06: {
        ...
        if (id === 1) {
          data.illuminance_alarm_config = {
            ...
            alarm_release_enable: ((b >> 7) & 1) === 1 ? 'enable' : 'disable',
      ...
      case 0xf1: {
        // target byte (0x08 = illuminance), skip it; read enable + i16LE value
        data.illuminance_calibration_settings = {
          enable:            bytes[offset+1] === 1 ? 'enable' : 'disable',
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      case 'set_illuminance_alarm': {
        const cfg  = p.illuminance_alarm_config ?? p;
      ...
      case 'set_illuminance_calibration': {
        const cfg = p.illuminance_calibration_settings ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const val = cfg.calibration_value ?? 0;
        const le  = val < 0 ? val + 0x10000 : val;
        bytes = [0xff, 0xf1, 0x08, en, le & 0xff, (le >> 8) & 0xff]; break;
      }
      case 'set_d2d_config': {
      ...
      default:
        throw new Error(`EM500-LGT: unsupported command "${type}"`);
    }
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // 0x03 0x94 (illumination uint32, lux) — unique to EM500-LGT.

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x03 && bytes[i+1] === 0x94) return true;
    }
    return false;
  }
}