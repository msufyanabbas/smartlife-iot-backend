// src/modules/devices/codecs/milesight/am319-well.codec.ts
/**
 * Milesight AM319 HCHO (WELL) — LoRaWAN Ambience Monitoring Sensor
 *
 * ── Relation to AM319 HCHO (IR) ──────────────────────────────────────────────
 * Both are AM319 variants measuring HCHO, but the sensor technology differs:
 *
 * WELL variant differences vs IR:
 *   HCHO channel:  0x0F 0x7D  (uint16 LE /10000 = ppm)    — replaces 0x0A 0x7D
 *   PM10:          absent — WELL has no PM10 channel
 *   History:       0x22/0x23 0xCE, 20B (no pm10 field)    — replaces 0x20/0x21
 *   Downlink adds: pm_self_clean_config (0xF9 0xAC)
 *                  pm_self_clean        (0xF9 0xAD)
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 ...
 *   0x0B 0x7D — pm2_5 (uint16 LE, µg/m³)
 *   0x0E 0x01 — buzzer_status (uint8: 0=off, 1=on)
 *   0x0F 0x7D — hcho (uint16 LE /10000, ppm)   ← WELL fingerprint
 *
 * ── History record layout (20B) ──────────────────────────────────────────────
 *   Channel IDs: 0x22 0xCE (tvoc=iaq) / 0x23 0xCE (tvoc=µg/m³)
 *   timestamp(4B) + temperature(2B) + humidity(2B) + pir(1B) + light_level(1B)
 *   + co2(2B) + tvoc(2B) + pressure(2B) + pm2_5(2B) + hcho(2B)
 *   NOTE: no pm10 in history
 *
 * ── Downlink channels ────────────────────────────────────────────────────────
 *   All AM319 IR channels except pm10-related, plus:
 *   0xF9 0xAC — pm_self_clean_config { enable(1B), period(1B, days) }
 *   0xF9 0xAD — pm_self_clean (trigger, 0xF9 0xAD 0xFF)
 *
 * ── canDecode fingerprint ─────────────────────────────────────────────────────
 *   0x0F 0x7D (HCHO ppm channel) — unique to WELL variant.
 *   No ordering constraint with AM319 IR (different fingerprint channels).
 */
...
const SCREEN_BITS: Record<string, number> = {
  temperature:0, humidity:1, co2:2, light:3, tvoc:4, smile:5, letter:6, pm2_5:7, pm10:8, hcho:9
};

export class MilesightAM319WellCodec extends BaseDeviceCodec {
  readonly codecId         = 'milesight-am319-well';
  readonly manufacturer    = 'Milesight';
  readonly supportedModels = ['AM319-HCHO'];
  readonly protocol        = 'lorawan' as const;
  readonly category        = 'Ambience Monitoring';
  readonly modelFamily     = 'AM319-HCHO';
  readonly imageUrl        = 'https://github.com/Milesight-IoT/SensorDecoders/raw/main/am-series/am319-hcho-well/am319.png';
  ...
  return {
    ...
    model:        'AM319-HCHO',
    description:  'Ambience Monitoring Sensor — Temperature, Humidity, PIR, Light Level, CO₂, TVOC, Pressure, PM2.5, and HCHO (WELL variant)',
    telemetryKeys: [
      ...
      // Note: no pm10 in WELL variant
      { key: 'hcho',        label: 'HCHO',        type: 'number' as const, unit: 'ppm'   },
    ],
    commands: [
      ...
      { type: 'pm_self_clean', label: 'Trigger PM Self-Clean', params: [] },
      {
      ...
      {
        type:   'set_pm_self_clean_config',
        label:  'Set PM Self-Clean Config',
        params: [
          { key: 'enable', label: 'Enable',         type: 'boolean' as const, required: true  },
          { key: 'period', label: 'Period (days)',   type: 'number'  as const, required: false, default: 1, min: 1, max: 255 },
        ],
      },
      {
    ...
    uiComponents: [
      ...
      { type: 'value' as const, label: 'HCHO',        keys: ['hcho'],        unit: 'ppm'    },
    ],
  };
}
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // WELL-exclusive HCHO channel: 0x0F 0x7D, unit: ppm, /10000
      else if (ch === 0x0f && ty === 0x7d) {
        decoded.hcho = (((bytes[i+1] << 8) | bytes[i]) & 0xffff) / 10000; i += 2;
      }

      // ── History records (20B — WELL, channel IDs 0x22/0x23, no pm10) ───────

      else if ((ch === 0x22 || ch === 0x23) && ty === 0xce) {
        if (!decoded.history) decoded.history = [];
        (decoded.history as any[]).push(this.decodeHistoryRecord(bytes, i, ch === 0x23));
        i += 20;
      }
      ...
      // ── Downlink response channels ─────────────────────────────────────────
        ...
        // PM self-clean commands use 0xF9 prefix
        const r = this.handleF9Response(ty, bytes, i);
  ...
  private decodeHistoryRecord(bytes: number[], i: number, luxMode: boolean): Record<string, any> {
    // Layout: ts(4)+temp(2)+hum(2)+pir(1)+light(1)+co2(2)+tvoc(2)+pres(2)+pm2_5(2)+hcho(2) = 20B
    ...
    return {
      ...
      hcho:        (((bytes[i+19] << 8) | bytes[i+18]) & 0xffff) / 1000,
    };
  }

  private handleF9Response(
    ty: number, bytes: number[], offset: number
  ): { data: Record<string, any>; offset: number } {
    const data: Record<string, any> = {};
    switch (ty) {
      case 0xac:
        data.pm_self_clean_config = {
          enable: bytes[offset] === 1 ? 'enable' : 'disable',
          period: bytes[offset + 1] & 0xff,
        };
        offset += 2; break;
      case 0xad:
        data.pm_self_clean = 'yes'; offset += 1; break;
      default:
        offset += 1; break;
    }
    return { data, offset };
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      // AM319 WELL: 3B only (same as IR, no 4B padding)
      ...
      // WELL-exclusive PM self-clean
      case 'set_pm_self_clean_config': {
        const cfg = p.pm_self_clean_config ?? p;
        const en  = cfg.enable === 'enable' || cfg.enable === 1 ? 1 : 0;
        const per = cfg.period ?? 1;
        if (per < 1 || per > 255) throw new Error('pm_self_clean_config.period: 1–255 days');
        bytes = [0xf9, 0xac, en, per & 0xff]; break;
      }
      case 'pm_self_clean': bytes = [0xf9, 0xad, 0xff]; break;
      ...
      default:
        throw new Error(`AM319-WELL: unsupported command "${type}"`);
    }
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // WELL-exclusive: 0x0F 0x7D (HCHO in ppm, vs IR's 0x0A 0x7D in mg/m³).
  // Also recognisable by history channel IDs 0x22/0x23 (vs IR's 0x20/0x21).
  // No ordering constraint with AM319 IR — fingerprints are disjoint.

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x0f && bytes[i+1] === 0x7d) return true; // HCHO ppm — WELL only
      if ((bytes[i] === 0x22 || bytes[i] === 0x23) && bytes[i+1] === 0xce) return true; // WELL history
    }
    return false;
  }
}