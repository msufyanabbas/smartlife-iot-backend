// src/modules/devices/codecs/milesight/am319.codec.ts
/**
 * Milesight AM319 — LoRaWAN Ambience Monitoring Sensor with HCHO + PM2.5/PM10
 *
 * ── Relation to AM308 ─────────────────────────────────────────────────────────
 * AM319 = AM308 + HCHO (formaldehyde) sensor
 *
 * Added vs AM308:
 *   Telemetry: 0x0A 0x7D — hcho (uint16 LE /100, mg/m³)   ← AM319-exclusive
 *   History:   22B records (AM308 = 20B) — appends hcho(2B) at end
 *   Screen:    adds `hcho` (bit9) to 0xF0 screen element bitmask
 *
 * Changed vs AM308:
 *   co2_abc_calibration_enable encoder: 3B only [0xFF 0x39 enable]
 *   AM308 encoder sends 7B with 4 zero bytes appended — AM319 omits them.
 *   Decoder handles both (reads 1B for enable, skips 4B in AM308 path).
 *   Since AM319 is a separate class, its decoder reads 1B (no skip).
 *
 * ── Telemetry channels ────────────────────────────────────────────────────────
 *   (All AM308 channels, plus:)
 *   0x0A 0x7D — hcho (uint16 LE /100, mg/m³)   ← AM319-exclusive fingerprint
 *
 * ── History record layout (22B) ──────────────────────────────────────────────
 *   timestamp(4B) + temperature(2B) + humidity(2B) + pir(1B) + light_level(1B)
 *   + co2(2B) + tvoc(2B) + pressure(2B) + pm2_5(2B) + pm10(2B) + hcho(2B)
 *
 * ── Downlink channels ────────────────────────────────────────────────────────
 *   Same as AM308 except:
 *   0xFF 0x39 — co2_abc_calibration_enable: 3B (no 4B padding)
 *   0xFF 0xF0 — screen elements bitmask includes hcho (bit9)
 *
 * ── canDecode fingerprint ─────────────────────────────────────────────────────
 *   0x0A 0x7D (HCHO) — exclusive to AM319 in this codec ecosystem.
 *   In ALL_CODECS: place AM319 BEFORE AM308 — AM319 payloads also contain
...
// AM319 screen element bits — adds hcho (bit9) vs AM308's 9-bit set
...
export class MilesightAM319Codec extends BaseDeviceCodec {
  readonly codecId: string         = 'milesight-am319';
  readonly manufacturer    = 'Milesight';
  readonly supportedModels = ['AM319', 'AM319-HCHO-IR'];
  ...
  decode(payload: string | Buffer, _fPort?: number): DecodedTelemetry {
    ...
    while (i < bytes.length) {
      ...
      // AM319-exclusive HCHO channel
      else if (ch === 0x0a && ty === 0x7d) {
        decoded.hcho = (((bytes[i+1] << 8) | bytes[i]) & 0xffff) / 100; i += 2;
      }
      ...
      // ── History records (22B — AM308 20B + hcho 2B) ───────────────────────
  ...
  private decodeHistoryRecord(bytes: number[], i: number, luxMode: boolean): Record<string, any> {
    ...
    return {
      ...
      hcho:        (((bytes[i+21] << 8) | bytes[i+20]) & 0xffff) / 100,
    };
  }
  ...
  private decodeDownlinkResponse(
    ...
    switch (ty) {
      ...
      // AM319: co2_abc response is 1B only (no 4B padding like AM308)
      case 0x39:
        data.co2_abc_calibration_enable = bytes[offset++] === 1 ? 'enable' : 'disable'; break;
      ...
      case 0xf0: {
        // AM319 adds hcho (bit9) to screen elements
  ...
  encode(command: { type: string; params?: any }): EncodedCommand {
    ...
    switch (type) {
      ...
      // AM319: 3B only — no 4B zero padding that AM308 sends
      ...
      case 'set_screen_display_elements': {
        // AM319 supports 10 elements including hcho
        let mask = 0, dval = 0;
      ...
      default:
        throw new Error(`AM319: unsupported command "${type}"`);
    }
  ...
  // ── canDecode ─────────────────────────────────────────────────────────────────
  // AM319 is uniquely identified by HCHO channel 0x0A 0x7D.
  // In ALL_CODECS: place AM319 BEFORE AM308 — AM319 payloads also contain
  // 0x0B/0x0C 0x7D (PM channels) which AM308 would match first.

  canDecode(payload: string | Buffer, _metadata?: any): boolean {
    const bytes = this.normalizePayload(payload);
    for (let i = 0; i + 1 < bytes.length; i++) {
      if (bytes[i] === 0x0a && bytes[i+1] === 0x7d) return true; // HCHO — AM319-exclusive
    }
    return false;
  }
}